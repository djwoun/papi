 ...
 *          djwoun@gmail.com
 *
 * AMD SMI native event API with device qualifiers (6-bit :device).
 */
...
typedef struct {
  int device;   /* 0..63 */
  int flags;    /* qlmask */
  int nameid;   /* index in native table */
} amds_event_info_t;
...
static int amds_strip_device_qualifier_impl(const char *name, char *base, int len)
{
  ...
  if (!p) {
    ...
    if ((int)n >= len) return PAPI_EBUF;
    memcpy(base, name, n + 1);
    return PAPI_OK;
  }

  int pos = 0;
  int head_len = (int)(p - name);
  if (head_len >= len) return PAPI_EBUF;
  memcpy(base + pos, name, (size_t)head_len);
  pos += head_len;

  const char *digits = p + 8; /* strlen(":device=") */
...
static int amds_name_to_device(const char *name, int *device)
{
  ...
  if (endp == digits) return PAPI_ENOEVNT;
...
static inline int amds_evt_id_create(const amds_event_info_t *info, unsigned int *event_id)
{
  ...
  unsigned int id = 0;
  id |= ((unsigned int)info->device << AMDS_DEVICE_SHIFT) & AMDS_DEVICE_MASK;
  id |= ((unsigned int)info->flags  << AMDS_QLMASK_SHIFT) & AMDS_QLMASK_MASK;
  id |= ((unsigned int)info->nameid << AMDS_NAMEID_SHIFT) & AMDS_NAMEID_MASK;
  *event_id = id;
  return PAPI_OK;
}
...
static inline int amds_evt_id_to_info(unsigned int event_id, amds_event_info_t *info)
{
  ...
  if (amds_strip_device_qualifier_impl(ntv->events[info->nameid].name, base, (int)sizeof(base)) != PAPI_OK)
    return PAPI_ENOEVNT;
  int ok = 0;
...
int amds_evt_code_to_name(unsigned int EventCode, char *name, int len)
{
  ...
  if (info.flags & AMDS_DEVICE_FLAG)
    snprintf(name, (size_t)len, "%s:device=%d", base, info.device);
  else
    snprintf(name, (size_t)len, "%s", base);
  return PAPI_OK;
}
...
int amds_evt_code_to_info(unsigned int EventCode, PAPI_event_info_t *info)
{
  ...
  switch (inf.flags) {
    ...
    case AMDS_DEVICE_FLAG:
      ...
      if (devices[0] != '\0') {
        snprintf(info->long_descr, sizeof(info->long_descr), "%s\nMandatory device qualifier [%s]",

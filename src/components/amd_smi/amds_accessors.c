...
int access_amdsmi_gpu_metrics(int mode, void *arg) {
  ...
  case 20: {
    ...
    if (event->subvariant >= gfxclk_count)
      return PAPI_ENOSUPP;
    event->value = metrics.current_gfxclks[event->subvariant];
    break;
  }
  case 21: {
    uint32_t socclk_count = sizeof(metrics.current_socclks) / sizeof(metrics.current_socclks[0]);
    if (event->subvariant >= socclk_count)
      return PAPI_ENOSUPP;
    event->value = metrics.current_socclks[event->subvariant];
    break;
  }
  default:
    return PAPI_ENOSUPP;
